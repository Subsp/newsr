...
OUTPUT_ROOT="/root/autodl-tmp/priors/kitchen_video_flashvsr_seqmat"
...
if [[ ! -d "${VIDEO_SR_REPO}" ]]; then
  echo "[video-prior-flashvsr-seqmat] repo not found: ${VIDEO_SR_REPO}"
  exit 1
fi

if [[ ! -x "${FLASHVSR_PYTHON}" ]]; then
  echo "[video-prior-flashvsr-seqmat] python not found or not executable: ${FLASHVSR_PYTHON}"
  exit 1
fi

if [[ ! -d "${FLASHVSR_MODEL_DIR}" ]]; then
  echo "[video-prior-flashvsr-seqmat] model folder missing: ${FLASHVSR_MODEL_DIR}"
  exit 1
fi

if [[ ! -d "${COLMAP_SPARSE_DIR}" ]]; then
  echo "[video-prior-flashvsr-seqmat] COLMAP sparse dir missing: ${COLMAP_SPARSE_DIR}"
  exit 1
fi
...
echo "[generate-video-prior-flashvsr-seqmat] done: ${OUTPUT_ROOT}/priors"
