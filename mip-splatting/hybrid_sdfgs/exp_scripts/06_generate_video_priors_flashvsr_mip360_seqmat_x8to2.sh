...
OUTPUT_ROOT="${OUTPUT_ROOT:-/root/autodl-tmp/priors/kitchen_video_flashvsr_${FLASHVSR_MODEL_TYPE}_seqmat_x8to2_raw}"
...
case "${FLASHVSR_MODEL_TYPE}" in
  tiny|tiny_long)
    ;;
  *)
    echo "[video-prior-flashvsr-seqmat-x8to2] unsupported FLASHVSR_MODEL_TYPE=${FLASHVSR_MODEL_TYPE}" >&2
    exit 1
    ;;
esac

if [[ ! -d "${VIDEO_SR_REPO}" ]]; then
  echo "[video-prior-flashvsr-seqmat-x8to2] repo not found: ${VIDEO_SR_REPO}"
  exit 1
fi

if [[ ! -x "${FLASHVSR_PYTHON}" ]]; then
  echo "[video-prior-flashvsr-seqmat-x8to2] python not found or not executable: ${FLASHVSR_PYTHON}"
  exit 1
fi

if [[ ! -d "${FLASHVSR_MODEL_DIR}" ]]; then
  echo "[video-prior-flashvsr-seqmat-x8to2] model folder missing: ${FLASHVSR_MODEL_DIR}"
  exit 1
fi

if [[ ! -d "${COLMAP_SPARSE_DIR}" ]]; then
  echo "[video-prior-flashvsr-seqmat-x8to2] COLMAP sparse dir missing: ${COLMAP_SPARSE_DIR}"
  exit 1
fi
...
echo "[generate-video-prior-flashvsr-seqmat-x8to2] done: ${OUTPUT_ROOT}/priors (model_type=${FLASHVSR_MODEL_TYPE})"
