...
def pseudo_sdf_from_anchors(
    ...
    if sigma_t is None:
        w = np.exp(-knn_d / nbr_sigma_n) * nbr_conf
    else:
        nbr_sigma_t = np.clip(sigma_t[knn_idx], 1e-4, None)
        tangent = np.stack([-nbr_n[..., 1], nbr_n[..., 0]], axis=-1)
        d_t = np.sum(delta * tangent, axis=-1)
        normed = np.sqrt((signed_plane / nbr_sigma_n) ** 2 + (d_t / nbr_sigma_t) ** 2)
        w = np.exp(-normed) * nbr_conf
...
def main():
    parser = argparse.ArgumentParser(description="Compare baseline sparse pseudo-SDF vs tangential support expansion.")
    parser.add_argument("--out_dir", type=str, default=".")
    parser.add_argument("--out_name", type=str, default="pseudo_sdf_tangential_expansion_compare.png")
    ...
    parser.add_argument("--tangential_scale_sparse", type=float, default=3.2)
    ...
    sigma_n = sigma.copy()
    sigma_t = sigma.copy()
    sigma_t[sparse_region] *= float(args.tangential_scale_sparse)
    conf_tan = conf.copy()
    conf_tan[sparse_region] *= 0.95
    pseudo_tan = pseudo_sdf_from_anchors(
        pts, anchors, normals, sigma_n, conf_tan, args.k_neighbors, sigma_t=sigma_t
    ).reshape(n, n)
    err_tan = np.abs(pseudo_tan - true_field)
    ...
    mae_tan_global = float(err_tan.mean())
    mae_base_sparse = float(err_base[sparse_mask].mean())
    mae_tan_sparse = float(err_tan[sparse_mask].mean())

    vmax = np.percentile(np.abs(true_field), 95)
    err_vmax = np.percentile(np.maximum(err_base, err_tan), 99)
    ...
    im2 = ax[0, 2].imshow(pseudo_tan, origin="lower", extent=[lo, hi, lo, hi], cmap="coolwarm", vmin=-vmax, vmax=vmax)
    ax[0, 2].contour(xx, yy, pseudo_tan, levels=[0.0], colors="k", linewidths=1.4)
    ax[0, 2].set_title(f"Pseudo-SDF (tangential x{args.tangential_scale_sparse:.1f})")
    ...
    im4 = ax[1, 1].imshow(err_tan, origin="lower", extent=[lo, hi, lo, hi], cmap="magma", vmin=0.0, vmax=err_vmax)
    ax[1, 1].contour(xx, yy, true_field, levels=[0.0], colors="cyan", linewidths=1.0)
    ax[1, 1].set_title(f"|Err| tangential expand\n(global {mae_tan_global:.4f}, sparse {mae_tan_sparse:.4f})")
    fig.colorbar(im4, ax=ax[1, 1], fraction=0.046, pad=0.04)

    im5 = ax[1, 2].imshow(err_tan - err_base, origin="lower", extent=[lo, hi, lo, hi], cmap="bwr", vmin=-err_vmax * 0.6, vmax=err_vmax * 0.6)
    ax[1, 2].contour(xx, yy, true_field, levels=[0.0], colors="k", linewidths=0.8)
    ax[1, 2].set_title("ΔErr = tangential - baseline\n(blue better, red worse)")
    ...
    print(
        ...
        f"tangent_mae_global={mae_tan_global:.6f} "
        f"baseline_mae_sparse={mae_base_sparse:.6f} "
        f"tangent_mae_sparse={mae_tan_sparse:.6f}"
    )
