...
#define RETURN_OR_INACTIVE() if constexpr(TILE_BASED_CULLING && LOAD_BALANCING) { active = false; } else { return; }
	...
	glm::vec3 cov2D = dilateCov2D(cov, splatting_settings.proper_ewa_scaling, kernel_size, det, convolution_scaling_factor);
	if (!isfinite(cov2D.x) || !isfinite(cov2D.y) || !isfinite(cov2D.z) || !isfinite(det) || !isfinite(convolution_scaling_factor))
		RETURN_OR_INACTIVE();
	if (det == 0.0f)
		RETURN_OR_INACTIVE();
	...
	if (!isfinite(co.x) || !isfinite(co.y) || !isfinite(co.z) || !isfinite(co.w))
		RETURN_OR_INACTIVE();
	...
	if (!isfinite(opacity_power_threshold))
		RETURN_OR_INACTIVE();
	...
	bb_center = mean2D;
	if (!isfinite(rect_dims.x) || !isfinite(rect_dims.y))
		RETURN_OR_INACTIVE();

	getRect(bb_center, rect_dims, rect_min, rect_max, grid);	
	const int rect_w = static_cast<int>(rect_max.x) - static_cast<int>(rect_min.x);
	const int rect_h = static_cast<int>(rect_max.y) - static_cast<int>(rect_min.y);
	const int max_tile_count = static_cast<int>(grid.x) * static_cast<int>(grid.y);
	if (rect_w <= 0 || rect_h <= 0)
		RETURN_OR_INACTIVE();
	const int tile_count_rect = rect_w * rect_h;
	if (tile_count_rect <= 0 || tile_count_rect > max_tile_count)
		RETURN_OR_INACTIVE();
	...
	float* out_color)
{
	...
	if (splatting_settings.sort_settings.sort_mode == SortMode::GLOBAL)
	{
		#define CALL_VANILLA(ENABLE_DEBUG_VIZ) renderCUDA<NUM_CHANNELS, ENABLE_DEBUG_VIZ> <<<grid, block>>> ( \
			ranges, point_list, W, H, focal_x, focal_y, subpixel_offset, splatting_settings.far_plane, means2D, colors, view2gaussian,conic_opacity, final_T, \
