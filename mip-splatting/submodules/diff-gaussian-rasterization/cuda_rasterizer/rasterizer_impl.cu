...
{
	GeometryState geom;
	const bool sizing_only = (chunk == nullptr);
	geom.scan_size = 0;
	...
	auto scan_err = cub::DeviceScan::InclusiveSum(
		nullptr,
		geom.scan_size,
		sizing_only ? reinterpret_cast<uint32_t*>(0x1000) : geom.tiles_touched,
		sizing_only ? reinterpret_cast<uint32_t*>(0x2000) : geom.tiles_touched,
		P
	);
	if (scan_err != cudaSuccess)
	{
		std::stringstream ss;
		ss << "GeometryState scan-size query failed: " << cudaGetErrorString(scan_err) << " (P=" << P << ")";
...
CudaRasterizer::BinningState CudaRasterizer::BinningState::fromChunk(char*& chunk, size_t P)
{
	...
	auto sort_err = cub::DeviceRadixSort::SortPairs(
		nullptr, binning.sorting_size,
		sizing_only ? reinterpret_cast<uint64_t*>(0x3000) : binning.point_list_keys_unsorted,
		sizing_only ? reinterpret_cast<uint64_t*>(0x4000) : binning.point_list_keys,
		sizing_only ? reinterpret_cast<uint32_t*>(0x5000) : binning.point_list_unsorted,
		sizing_only ? reinterpret_cast<uint32_t*>(0x6000) : binning.point_list,
		P);
	if (sort_err != cudaSuccess)
	{
		std::stringstream ss;
		ss << "BinningState sort-size query failed: " << cudaGetErrorString(sort_err) << " (P=" << P << ")";
		throw std::runtime_error(ss.str());
	}
...
CudaRasterizer::PointBinningState CudaRasterizer::PointBinningState::fromChunk(char*& chunk, size_t PN)
{
	...
	auto sort_err = cub::DeviceRadixSort::SortPairs(
		nullptr, binning.sorting_size,
		sizing_only ? reinterpret_cast<uint64_t*>(0x7000) : binning.point_list_keys_unsorted,
		sizing_only ? reinterpret_cast<uint64_t*>(0x8000) : binning.point_list_keys,
		sizing_only ? reinterpret_cast<uint32_t*>(0x9000) : binning.point_list_unsorted,
		sizing_only ? reinterpret_cast<uint32_t*>(0xA000) : binning.point_list,
		PN);
	if (sort_err != cudaSuccess)
	{
		std::stringstream ss;
		ss << "PointBinningState sort-size query failed: " << cudaGetErrorString(sort_err) << " (PN=" << PN << ")";
		throw std::runtime_error(ss.str());
	}
	obtain(chunk, binning.list_sorting_space, binning.sorting_size, 128);
	return binning;
...
CudaRasterizer::PointState CudaRasterizer::PointState::fromChunk(char*& chunk, size_t P)
{
	...
	auto scan_err = cub::DeviceScan::InclusiveSum(
		nullptr,
		geom.scan_size,
		sizing_only ? reinterpret_cast<uint32_t*>(0xB000) : geom.tiles_touched,
		sizing_only ? reinterpret_cast<uint32_t*>(0xC000) : geom.tiles_touched,
		P
	);
	if (scan_err != cudaSuccess)
	{
		std::stringstream ss;
		ss << "PointState scan-size query failed: " << cudaGetErrorString(scan_err) << " (P=" << P << ")";
		throw std::runtime_error(ss.str());
	}
	obtain(chunk, geom.scanning_space, geom.scan_size, 128);
	obtain(chunk, geom.point_offsets, P, 128);
	...
	bool debug)
{
	...
	const size_t max_geometry_chunk_size = size_t(8) * 1024 * 1024 * 1024;
	if (chunk_size == 0 || chunk_size > max_geometry_chunk_size)
	{
		std::stringstream ss;
		ss << "Merged rasterizer produced an implausible geometry chunk size: " << chunk_size
		   << " bytes (P=" << P
		   << ", requires_cov3D_inv=" << int(requires_cov3D_inv) << ")";
		throw std::runtime_error(ss.str());
	}
	char* chunkptr = geometryBuffer(chunk_size);
	GeometryState geomState = GeometryState::fromChunk(chunkptr, P, requires_cov3D_inv);
	const size_t max_scan_size = size_t(2) * 1024 * 1024 * 1024;
	if (geomState.scan_size == 0 || geomState.scan_size > max_scan_size)
	{
		std::stringstream ss;
		ss << "Merged rasterizer produced an implausible geometry scan size: " << geomState.scan_size
		   << " bytes (P=" << P << ")";
		throw std::runtime_error(ss.str());
	}
	...
	const size_t max_image_chunk_size = size_t(8) * 1024 * 1024 * 1024;
	if (img_chunk_size == 0 || img_chunk_size > max_image_chunk_size)
	{
		std::stringstream ss;
		ss << "Merged rasterizer produced an implausible image chunk size: " << img_chunk_size
	...
	const long long max_possible_rendered =
		static_cast<long long>(P) *
		static_cast<long long>(tile_grid.x) *
		static_cast<long long>(tile_grid.y);
	if (num_rendered < 0 || static_cast<long long>(num_rendered) > max_possible_rendered)
	{
		std::stringstream ss;
		ss << "Merged rasterizer produced an invalid duplicated-Gaussian count: num_rendered="
		   << num_rendered
		   << " (P=" << P
		   << ", tile_grid=" << tile_grid.x << "x" << tile_grid.y
		   << ", max_possible=" << max_possible_rendered << ")";
		throw std::runtime_error(ss.str());
	}
