...
LAMBDA_SURFACE_HF_EDGE_SUPPORT="${LAMBDA_SURFACE_HF_EDGE_SUPPORT:-0.0}"
LAYER_FREQUENCY_SURFACE_HF_SUPPORT_GAIN="${LAYER_FREQUENCY_SURFACE_HF_SUPPORT_GAIN:-4.0}"
LAYER_FREQUENCY_SURFACE_HF_SUPPORT_COVERAGE_WEIGHT="${LAYER_FREQUENCY_SURFACE_HF_SUPPORT_COVERAGE_WEIGHT:-0.25}"
...
PRIOR_EDGE_SUBSET_MODE="${PRIOR_EDGE_SUBSET_MODE:-none}"
PRIOR_EDGE_FULL_LOSS_WEIGHT="${PRIOR_EDGE_FULL_LOSS_WEIGHT:-1.0}"
PRIOR_EDGE_SUBSET_ENERGY_WEIGHT="${PRIOR_EDGE_SUBSET_ENERGY_WEIGHT:-0.0}"
PRIOR_EDGE_SUBSET_LOWFREQ_WEIGHT="${PRIOR_EDGE_SUBSET_LOWFREQ_WEIGHT:-0.0}"
PRIOR_EDGE_SUBSET_OPACITY_FLOOR="${PRIOR_EDGE_SUBSET_OPACITY_FLOOR:-0.0}"
PRIOR_EDGE_SUBSET_OPACITY_FLOOR_WEIGHT="${PRIOR_EDGE_SUBSET_OPACITY_FLOOR_WEIGHT:-0.0}"
PRIOR_EDGE_SUBSET_MIN_GAUSSIANS="${PRIOR_EDGE_SUBSET_MIN_GAUSSIANS:-16}"
...
PRIOR_HF_SEED_SHAPE_ORIENTATION_RADIUS_PX="${PRIOR_HF_SEED_SHAPE_ORIENTATION_RADIUS_PX:-0}"
...
PRIOR_HF_SEED_NMS_RADIUS_PX="${PRIOR_HF_SEED_NMS_RADIUS_PX:-0}"
...
if is_nonzero "${LAMBDA_NON_SURFACE_HF}" \
  ...
  || is_nonzero "${LAMBDA_SURFACE_HF_EDGE_SUPPORT}" \
...
if [[ -n "${PRIOR_EDGE_DIR}" || -n "${PRIOR_EDGE_MASK_DIR}" ]]; then
  ...
  echo "[nosr-layerfreq-cleanup-v0] prior edge      : target=${PRIOR_EDGE_DIR} mask=${PRIOR_EDGE_MASK_DIR} lambda=${LAMBDA_PRIOR_EDGE} mode=${PRIOR_EDGE_LOSS_MODE} subset=${PRIOR_EDGE_SUBSET_MODE} full_w=${PRIOR_EDGE_FULL_LOSS_WEIGHT} subset_energy=${PRIOR_EDGE_SUBSET_ENERGY_WEIGHT} subset_lf=${PRIOR_EDGE_SUBSET_LOWFREQ_WEIGHT} subset_opacity_floor=${PRIOR_EDGE_SUBSET_OPACITY_FLOOR}/${PRIOR_EDGE_SUBSET_OPACITY_FLOOR_WEIGHT}"
...
if [[ "${FORCE_RERUN}" == "1" || ! -f "${TRAIN_DONE_PATH}" ]]; then
  ...
  if [[ "${NEED_LAYER_FREQUENCY}" == "1" ]]; then
    TRAIN_ARGS+=(
      ...
      --lambda_surface_hf_edge_support "${LAMBDA_SURFACE_HF_EDGE_SUPPORT}"
      --layer_frequency_surface_hf_support_gain "${LAYER_FREQUENCY_SURFACE_HF_SUPPORT_GAIN}"
      --layer_frequency_surface_hf_support_coverage_weight "${LAYER_FREQUENCY_SURFACE_HF_SUPPORT_COVERAGE_WEIGHT}"
  ...
  if [[ -n "${PRIOR_EDGE_DIR}" || -n "${PRIOR_EDGE_MASK_DIR}" ]]; then
    TRAIN_ARGS+=(
      ...
      --prior_edge_subset_mode "${PRIOR_EDGE_SUBSET_MODE}"
      --prior_edge_full_loss_weight "${PRIOR_EDGE_FULL_LOSS_WEIGHT}"
      --prior_edge_subset_energy_weight "${PRIOR_EDGE_SUBSET_ENERGY_WEIGHT}"
      --prior_edge_subset_lowfreq_weight "${PRIOR_EDGE_SUBSET_LOWFREQ_WEIGHT}"
      --prior_edge_subset_opacity_floor "${PRIOR_EDGE_SUBSET_OPACITY_FLOOR}"
      --prior_edge_subset_opacity_floor_weight "${PRIOR_EDGE_SUBSET_OPACITY_FLOOR_WEIGHT}"
      --prior_edge_subset_min_gaussians "${PRIOR_EDGE_SUBSET_MIN_GAUSSIANS}"
  ...
  if [[ "${PRIOR_HF_SEED_ENABLE}" == "1" ]]; then
    TRAIN_ARGS+=(
      ...
      --prior_hf_seed_shape_orientation_radius_px "${PRIOR_HF_SEED_SHAPE_ORIENTATION_RADIUS_PX}"
      ...
      --prior_hf_seed_nms_radius_px "${PRIOR_HF_SEED_NMS_RADIUS_PX}"
