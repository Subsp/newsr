...
if [[ -n "${PRIOR_EDGE_DIR}" || -n "${PRIOR_EDGE_MASK_DIR}" ]]; then
  ...
  echo "[nosr-layerfreq-cleanup-v0] prior edge      : target=${PRIOR_EDGE_DIR} mask=${PRIOR_EDGE_MASK_DIR} lambda=${LAMBDA_PRIOR_EDGE} mode=${PRIOR_EDGE_LOSS_MODE} subset=${PRIOR_EDGE_SUBSET_MODE} full_w=${PRIOR_EDGE_FULL_LOSS_WEIGHT} subset_lf=${PRIOR_EDGE_SUBSET_LOWFREQ_WEIGHT}"
