...
if [[ "${LAMBDA_PRIOR_LOCAL_SURFACE}" != "0" && "${LAMBDA_PRIOR_LOCAL_SURFACE}" != "0.0" ]]; then
  echo "[nosr-layerfreq-cleanup-v0] local surface  : payload=${PRIOR_LOCAL_SURFACE_PAYLOAD} knn=${PRIOR_LOCAL_SURFACE_KNN} max_samples=${PRIOR_LOCAL_SURFACE_MAX_SAMPLES} normal_cos=${PRIOR_LOCAL_SURFACE_NORMAL_MIN_COS}"
fi
