...
LAMBDA_PRIOR_EDGE_ENTON="${LAMBDA_PRIOR_EDGE_ENTON:-0.0}"
LAMBDA_PRIOR_EDGE_LINEGS="${LAMBDA_PRIOR_EDGE_LINEGS:-0.0}"
PRIOR_EDGE_STRUCTURE_MIN_GAUSSIANS="${PRIOR_EDGE_STRUCTURE_MIN_GAUSSIANS:-64}"
PRIOR_EDGE_STRUCTURE_MAX_SAMPLES="${PRIOR_EDGE_STRUCTURE_MAX_SAMPLES:-2048}"
PRIOR_EDGE_STRUCTURE_KNN="${PRIOR_EDGE_STRUCTURE_KNN:-8}"
PRIOR_EDGE_ENTON_MAX_ENTROPY="${PRIOR_EDGE_ENTON_MAX_ENTROPY:-0.58}"
PRIOR_EDGE_ENTON_SCALE_WEIGHT="${PRIOR_EDGE_ENTON_SCALE_WEIGHT:-0.25}"
PRIOR_EDGE_ENTON_LOCAL_WEIGHT="${PRIOR_EDGE_ENTON_LOCAL_WEIGHT:-1.0}"
PRIOR_EDGE_LINEGS_THIN_RATIO="${PRIOR_EDGE_LINEGS_THIN_RATIO:-0.35}"
PRIOR_EDGE_LINEGS_LINE_RATIO="${PRIOR_EDGE_LINEGS_LINE_RATIO:-0.45}"
PRIOR_EDGE_LINEGS_SCALE_WEIGHT="${PRIOR_EDGE_LINEGS_SCALE_WEIGHT:-1.0}"
PRIOR_EDGE_LINEGS_LOCAL_WEIGHT="${PRIOR_EDGE_LINEGS_LOCAL_WEIGHT:-0.5}"
PRIOR_EDGE_LINEGS_MAX_AXIS="${PRIOR_EDGE_LINEGS_MAX_AXIS:-0.0}"
...
if [[ -n "${PRIOR_EDGE_DIR}" || -n "${PRIOR_EDGE_MASK_DIR}" ]]; then
  ...
  echo "[nosr-layerfreq-cleanup-v0] edge structure  : enton=${LAMBDA_PRIOR_EDGE_ENTON} linegs=${LAMBDA_PRIOR_EDGE_LINEGS} samples=${PRIOR_EDGE_STRUCTURE_MAX_SAMPLES} knn=${PRIOR_EDGE_STRUCTURE_KNN} entropy=${PRIOR_EDGE_ENTON_MAX_ENTROPY}"
...
if [[ "${FORCE_RERUN}" == "1" || ! -f "${TRAIN_DONE_PATH}" ]]; then
  ...
  if [[ -n "${PRIOR_EDGE_DIR}" || -n "${PRIOR_EDGE_MASK_DIR}" ]]; then
    TRAIN_ARGS+=(
      ...
      --lambda_prior_edge_enton "${LAMBDA_PRIOR_EDGE_ENTON}"
      --lambda_prior_edge_linegs "${LAMBDA_PRIOR_EDGE_LINEGS}"
      --prior_edge_structure_min_gaussians "${PRIOR_EDGE_STRUCTURE_MIN_GAUSSIANS}"
      --prior_edge_structure_max_samples "${PRIOR_EDGE_STRUCTURE_MAX_SAMPLES}"
      --prior_edge_structure_knn "${PRIOR_EDGE_STRUCTURE_KNN}"
      --prior_edge_enton_max_entropy "${PRIOR_EDGE_ENTON_MAX_ENTROPY}"
      --prior_edge_enton_scale_weight "${PRIOR_EDGE_ENTON_SCALE_WEIGHT}"
      --prior_edge_enton_local_weight "${PRIOR_EDGE_ENTON_LOCAL_WEIGHT}"
      --prior_edge_linegs_thin_ratio "${PRIOR_EDGE_LINEGS_THIN_RATIO}"
      --prior_edge_linegs_line_ratio "${PRIOR_EDGE_LINEGS_LINE_RATIO}"
      --prior_edge_linegs_scale_weight "${PRIOR_EDGE_LINEGS_SCALE_WEIGHT}"
      --prior_edge_linegs_local_weight "${PRIOR_EDGE_LINEGS_LOCAL_WEIGHT}"
      --prior_edge_linegs_max_axis "${PRIOR_EDGE_LINEGS_MAX_AXIS}"
    )
