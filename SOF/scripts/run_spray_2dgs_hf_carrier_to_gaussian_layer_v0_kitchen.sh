...
OUTPUT_NAME="${OUTPUT_NAME:-${BASE_EXPERIMENT_NAME}_spray_2dgs_effective_hf_gaulayer_v0}"
...
FRONT_OFFSET_PX="${FRONT_OFFSET_PX:-0.35}"
...
OPACITY_FLOOR="${OPACITY_FLOOR:-0.02}"
OPACITY_SCALE="${OPACITY_SCALE:-0.12}"
...
for required in "${BASE_MODEL_DIR}" "${BASE_PLY}" "${PRIMITIVE_DIR}" "${EVIDENCE_RGB_DIR}" "${EVIDENCE_WEIGHT_DIR}"; do
  if [[ ! -e "${required}" ]]; then
    echo "[spray-2dgs-to-gaussian-layer-v0] required path not found: ${required}" >&2
    exit 1
  fi
done
...
echo "[spray-2dgs-to-gaussian-layer-v0] base model : ${BASE_MODEL_DIR}"
echo "[spray-2dgs-to-gaussian-layer-v0] base ply   : ${BASE_PLY}"
echo "[spray-2dgs-to-gaussian-layer-v0] primitives : ${PRIMITIVE_DIR}"
echo "[spray-2dgs-to-gaussian-layer-v0] evidence   : ${EVIDENCE_RGB_DIR}"
echo "[spray-2dgs-to-gaussian-layer-v0] weight     : ${EVIDENCE_WEIGHT_DIR}"
...
"${PYTHON_BIN}" "${SOF_ROOT}/scripts/spray_2dgs_hf_carrier_to_gaussian_layer_v0.py" "${SPRAY_ARGS[@]}"

NEWBORN_PLY="${NEWBORN_MODEL_DIR}/point_cloud/iteration_${BASE_ITERATION}/point_cloud.ply"
if [[ ! -f "${NEWBORN_PLY}" ]]; then
  echo "[spray-2dgs-to-gaussian-layer-v0] newborn PLY not found after lift: ${NEWBORN_PLY}" >&2
  exit 1
fi
...
if [[ ! -f "${MERGE_SCRIPT}" ]]; then
  echo "[spray-2dgs-to-gaussian-layer-v0] merge script not found: ${MERGE_SCRIPT}" >&2
  exit 1
fi
echo "[spray-2dgs-to-gaussian-layer-v0] merge     : ${MERGE_SCRIPT}"
...
echo "[spray-2dgs-to-gaussian-layer-v0] tags:"
echo "  ${OUTPUT_MODEL_DIR}/point_cloud/iteration_${OUTPUT_ITERATION}/gaussian_tags.pt"
echo "[spray-2dgs-to-gaussian-layer-v0] summary:"
echo "  ${OUTPUT_MODEL_DIR}/spray_2dgs_hf_carrier_to_gaussian_layer_v0_summary.json"
