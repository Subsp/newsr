...
def copy_render_config(src_model_path: str, dst_model_path: Path) -> None:
...
def merge_models(base: GaussianModel, extra: GaussianModel) -> GaussianModel:
    ...
    merged._features_dc = nn.Parameter(
        torch.cat((base._features_dc.detach(), extra._features_dc.detach()), dim=0),
        requires_grad=False,
    )
    merged._features_rest = nn.Parameter(
        torch.cat((base._features_rest.detach(), extra._features_rest.detach()), dim=0),
        requires_grad=False,
    )
    merged._opacity = nn.Parameter(
        torch.cat((base._opacity.detach(), extra._opacity.detach()), dim=0),
        requires_grad=False,
    )
    merged._scaling = nn.Parameter(
        torch.cat((base._scaling.detach(), extra._scaling.detach()), dim=0),
        requires_grad=False,
    )
    merged._rotation = nn.Parameter(
        torch.cat((base._rotation.detach(), extra._rotation.detach()), dim=0),
        requires_grad=False,
    )
