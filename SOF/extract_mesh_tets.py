...
def marching_tetrahedra_with_binary_search(model_path, name, iteration, views, gaussians : GaussianModel, pipeline, background, kernel_size, meshing_settings : MeshingSettings, splat_args : ExtendedSettings):
    ...
    # starting now, we don't need the exact depth anymore, lets accelerate
    ...
    for step in range(n_binary_steps):
        ...
        # filter
        if meshing_settings.filter_mesh:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bounding_mode", type=lambda sortmode: BoundingSetting[sortmode], choices=list(BoundingSetting), default=BoundingSetting.SIGMA_333)
    ...
    parser.add_argument("--opacity_cutoff_tetra", default=0., type=float)
    ...
    mesh_settings = MeshingSettings(args.near, args.far, args.texture_mesh, args.filter_mesh, args.bounding_mode, args.mesh_name,
