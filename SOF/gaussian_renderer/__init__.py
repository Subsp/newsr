...
def _build_raster_settings(
    ...
    *,
    ...
    subpixel_offset: torch.Tensor | None = None,
):
    kwargs = {
    ...
    field_names = set(getattr(GaussianRasterizationSettings, "_fields", ()))
    if "kernel_size" in field_names:
        kwargs["kernel_size"] = float(kernel_size)
    if "subpixel_offset" in field_names:
        ...
        kwargs["subpixel_offset"] = subpixel_offset
...
def render(
    ...
    *,
    ...
    scales = None
    opacity = None
    if bool(vanilla_mip_mode):
        filter_3d = None
        scales = pc.get_scaling_with_3D_filter
        opacity = pc.get_opacity_with_3D_filter
    elif pipe.compute_filter3D_python:
        filter_3d = None
        scales = pc.get_scaling_with_3D_filter
        opacity = pc.get_opacity_with_3D_filter
    else:
    ...
    if pipe.compute_view2gaussian_python and not bool(vanilla_mip_mode):
...
def render_simple(
    ...
    override_color = None,
    splat_args: ExtendedSettings = None,
    debug_data : DebugVisualization = None,
    *,
    kernel_size: float = 0.0,
    subpixel_offset: torch.Tensor | None = None,
    vanilla_mip_mode: bool = False,
