...
class SplattingSettings():
    ...
    def _apply_config_value(self, target, key, value):
        if isinstance(value, dict):
            nested_target = getattr(target, key, None)
            if nested_target is None:
                return
            for nested_key, nested_value in value.items():
                self._apply_config_value(nested_target, nested_key, nested_value)
            return
    ...
    def _load_settings_from_config_dict(self, config):
        settings = ExtendedSettings()
        for key, value in config.items():
            self._apply_config_value(settings, key, value)
        return settings

    def _load_settings_from_json(self, json_path):
        with open(json_path, "r") as json_file:
            config = json.load(json_file)
        return self._load_settings_from_config_dict(config)
    ...
    def get_settings(self, arguments):
        ...
        if self.render:
            ...
            if os.path.exists(cfgfilepath):
                ...
                self.settings = self._load_settings_from_json(cfgfilepath)
            else:
            ...
            for arg in vars(arguments).items():
                if any([arg[0] in z.option_strings[0] for z in self.group_config._group_actions]):
                    ...
                    with open(arg[1], 'r') as json_file:
                        config = json.load(json_file)
                        self.settings = self._load_settings_from_config_dict(config)
    ...
    def get_settings_from_path(self, path):
        ...
        while path_to_start:
            ...
            if os.path.exists(config_path):
                return self._load_settings_from_json(config_path)
